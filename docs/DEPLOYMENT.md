...
- **로그인** — 사용자가 직접 정하는 아이디/비밀번호(bcrypt) + 인계 코드. 아래 항목 참고.
...
기기를 바꾸면 코인이 날아가는 문제를 푸는 부분이다. 두 경로가 있고 서로를 채운다.

| 수단 | 무엇 | 잃어버렸을 때 |
|---|---|---|
| 아이디 + 비밀번호 | 사용자가 직접 정한다. 평소 쓰는 길 | 인계 코드로 되찾는다 |
| 인계 코드 | 서버가 만든 12자(`K7M2-9QPX-3W5B`). 적어 두는 물건 | 로그인해서 재발급(= 옛 코드 폐기) |

지켜야 하는 것:

- **로그인은 승격이지 새 계정 생성이 아니다.** `POST /api/me/credentials` 는 지금 쓰던
  익명 계정에 아이디를 붙인다 — 그래서 코인·아이템이 그대로 따라오고, 회원가입 화면이 없다.
- **bcrypt 를 이벤트 루프에서 직접 부르지 않는다.** 이 프로세스의 루프는 60Hz 틱 루프와
  같은 루프다. 해싱 한 번이 60~100ms 이므로 동기로 부르면 틱이 예닐곱 번 밀린다.
  `app/services/passwords.py` 가 `asyncio.to_thread` 로 빼고, `tests/test_passwords.py` 가
  그 우회가 걷히면 실패한다.
- **로그인 창구는 레이트리밋이 걸려 있다**(IP 기준 10분 10회, `app/services/ratelimit.py`).
  인스턴스 1대라 인메모리다 — Phase 3(가로 확장)에 들어가면 Redis 로 옮겨야 한다.
- 실패 사유를 "아이디 없음 / 비번 틀림"으로 쪼개지 않는다. 쪼개면 아이디 존재 여부가 샌다.
- 아이디/비밀번호는 **선택 사항**이다. 안 만들어도 게임은 예전 그대로 돌아간다.

**⚠ 병합은 아직 없다.** 다른 계정으로 로그인하면 이 브라우저에 있던 익명 계정의 진행은
따라가지 않는다(계정 자체는 DB 에 남지만 접근 경로를 잃는다). 프런트가 로그인 폼에서
이 사실을 경고한다. 코인 합산 병합을 넣으려면 **`ACCOUNT_SEED_COINS_MAX=0` 이 선행돼야
한다** — 아니면 씨앗 코인으로 익명 계정을 찍어 병합하는 무한 코인 경로가 열린다.

**아직 남은 것**: 레벨/경험치 보상, 전적·랭킹(`accounts.record_match_result()` 는 구현돼
있지만 아직 호출부가 없다 — 매치 종료 훅이 필요하다), 소셜 로그인, 그리고 이관이 끝난 뒤

...
async def create_room(body: CreateRoomRequest) -> CreateRoomResponse:
    try:
        room = room_manager.create(mode=body.mode, max_players=body.max_players)
    except RoomError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    return CreateRoomResponse(code=room.code, mode=room.mode, max_players=room.max_players)

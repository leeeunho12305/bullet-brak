// 로비의 경쟁전 카드 — 내 티어 / RR 게이지 / 시즌 성적.
//
// 이 카드는 **경쟁전을 쓸 수 있는 서버에서만** 그려진다(store 의 myRank 가 null 이면
// 로비가 아예 렌더하지 않는다). 그래서 여기서는 "DB 가 없는 경우"를 다루지 않는다.
import { memo } from 'react';
import type { JSX } from 'react';
import RankBadge from '@/components/RankBadge';
import { rankLabel, streakLabel, tierOf, winRate } from '@/api/ranked';
...
function RankPanelInner({ rank, onOpenBoard }: Props): JSX.Element {
  const { rank: stats, position, season } = rank;
  ...
  return (
    <div className="rank-card">
      ...
      <button type="button" className="btn btn-ghost btn-block" onClick={onOpenBoard}>
        🏆 {season.name} 순위표 · 전적
      </button>
    </div>
  );

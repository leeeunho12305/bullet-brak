...
# 맵/스폰은 layout 이 담당한다. PROTOCOL §5 는 이 함수들을 engine 의 것으로 적어 뒀고
# 호출부(ws.py·rooms.py)도 engine 으로 부르므로 여기서 그대로 다시 내보낸다.
__all__ = [
    "clear_platforms",
    "forget_room",
    "open_card_pick",
...
def _check_round_over(room: Room) -> None:
    ...
    if winner:
        ...
        winner.coins_earned += C.COINS_ROUND_WIN
...
def _resolve_round_over(room: Room) -> None:
    ...
    if room.scores[winner.id] >= C.SCORE_TO_WIN:
        ...
        winner.coins_earned += C.COINS_MATCH_WIN
        room.bullets.clear()
        room.zones.clear()
...
def start_game(room: Room) -> bool:
    ...
    경쟁전은 두 명이 다 있어야 시작한다 — 혼자 시작해서 상대가 들어오면 랭크가 걸린
    판이 이미 진행 중인 상태가 된다.
    """
